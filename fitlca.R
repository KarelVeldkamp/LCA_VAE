...
for (NCLASS in c(10)){
  for (NREP in c(1, 10, 20)){
    for (iteration in 1:50){
      # read data and true parameters
      data = read.csv(paste0(c('~/Documents/GitHub/LCA_VAE/true/data/data_', NCLASS, '_', iteration, '.csv'), collapse = ''), header = F) +1
      true_class = read.csv(paste0(c('true/parameters/class_',NCLASS,'_',iteration,'.csv'), collapse = ''), header=F)
      true_probs = as.matrix(read.csv(paste0(c('true/parameters/probs_',NCLASS,'_',iteration,'.csv'), collapse = ''), header=F))
      ...
      print(paste0(c('results/estimates/est_lca_', NCLASS, '_', NREP, '_', iteration, '.csv'), collapse=''))
      write.csv(results, paste0(c('results/estimates/est_lca_', NCLASS, '_', NREP, '_', iteration, '.csv'), collapse=''))
      
      # write metrics to file

      fileConn<-file(paste0(c('results/metrics/lca_', NCLASS, '_', NREP, '_', iteration, '.txt'), collapse=''))
